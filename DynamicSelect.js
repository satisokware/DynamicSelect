class DynamicSelect {
	constructor(element, options = {}) {
		...
		this.name = this.selectElement.getAttribute('name') ?
			this.selectElement.getAttribute('name') : 'dynamic-select-' + Math.floor(Math.random() * 1000000);
...
`;
		...
		// return the actual widget root element
	...
	_eventHandlers() {
		this.element.querySelectorAll('.dynamic-select-option').forEach(option => {
			option.onclick = () => {
				...
				const matched = this.data.filter(data => data.value == option.getAttribute('data-value'));
				if (matched.length) matched[0].selected = true;
				this.element.querySelector('.dynamic-select-header').classList.remove('dynamic-select-header-active');
				this.options.onChange(option.getAttribute('data-value'),
					option.querySelector('.dynamic-select-option-text') ?
						option.querySelector('.dynamic-select-option-text').innerHTML : '', option);
			};
		});
		...
		document.addEventListener('click', event => {
			if (!event.target.closest('.' + this.name) && !(this.selectElement.id && event.target.closest('label[for="' + this.selectElement.id + '"]'))) {
				this.element.querySelector('.dynamic-select-header').classList.remove('dynamic-select-header-active');
			}
		});
	}
	_updateSelected() {
		if (this.selectedValue) {
			const sel = this.element.querySelector('.dynamic-select-selected');
			if (sel) {
				this.element.querySelector('.dynamic-select-header').innerHTML = sel.innerHTML;
